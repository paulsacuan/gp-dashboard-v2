...
export default function ProductUpdate() {
  ...
  const [state, setState] = useState({
    ...
    nextDayDeliveryAllowed: product.nextday_delivery_allowed,
    expressDeliveryAllowed: product.express_delivery_allowed,
    btc_availability: product.btc_availability,
    garage_type: product.garage_type,
  ...
  useEffect(() => {
    ...
    if (product.parent) {
      setSelectedParent(product.parent);
    }
  }, [product, reset]);
  ...
  const onSubmit = async data => {
    try {
      ...
      const response = await axios.put(
        `${apiUrl}/v2/products/update/${data.id}`,
        data,
        {
          headers: {
