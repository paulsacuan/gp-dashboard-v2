...
export default function GarageUserView() {
  ...
  const onSubmit = async data => {
    try {
      ...
      if (response.status === 200) {
        setLoading(false);
        setIsEditMode(false);
        toast.success('Garage User Updated!', {
          position: 'top-right',
          autoClose: 5000,
        ...
        navigate(`/garages/${data.id}/users/${user.id}`, { replace: true });
      } else {
        setLoading(false);
        setIsEditMode(false);

        toast.error('Garage user update failed!', {
          position: 'top-right',
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
            ...
            {isEditMode ? (
              ...
              <div className="my-2 flex items-center justify-between">
                <button
                  onClick={e => {
                    e.preventDefault();
                    navigate(`/garages/${user.garage_id}/users`);
                  }}
                  className="relative inline-block px-4 py-2 font-bold group w-full max-w-[8rem]"
                >
        ...
        <Card className="flex-1 max-w-full p-4 grid grid-cols-4 gap-2 text-xs hover:shadow-md">
          <div className="col-span-4 text-left my-4 relative">
            <p className="text-secondary-500 text-md text-lg font-semibold">
              {isEditMode ? 'Edit User' : 'Garage user details'}
            </p>
          </div>
